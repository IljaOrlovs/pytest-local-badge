"""Session badges — counts and timings sourced from the pytest run itself.

Everything in here reads `session.testscollected`, `session.testsfailed`,
or the `terminalreporter` plugin's `stats` dict. No package metadata, no
user-supplied input — just what pytest already knows after the suite has
finished running.
"""
